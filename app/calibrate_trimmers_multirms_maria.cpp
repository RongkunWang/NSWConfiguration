...
std::pair<float,int> find_linear_region_slope(nsw::ConfigSender & cs,
      ...
      for (auto trim : trims) {
        ...
        if ( sample_to_mV(ch_baseline_med) == 0 ) continue;
...
bool check_disconnected_channel(const char mmfe8[15], int vmm_id, int channel_id){
    ...
    //large
    if((L == 0 || L == 1 || L == 6 || L == 7) && (i<72 || (i>5047 && i<5167) || i>8143)) return true;
    if((L == 2 || L == 3 || L == 4 || L == 5) && (i<86 || (i>5083 && i<5162) || i>8148)) return true;
...
int main(int ac, const char *av[]) {
  ...
  for (auto & feb : frontend_configs) { // big feb loop
    ...
    for (int channel_id = 0; channel_id < NCH_PER_VMM; channel_id++){// channel loop
      ...
      if ( (n_samples*10 < 200) || (n_samples*10 > 600)) 
          continue;
      ...
      if (sample_to_mV(mean) < 100 || sample_to_mV(mean) > 250) 
          continue;
      ...
      for (unsigned int i = 0; i < results.size(); i++) {
        ...
        std::cout<<" maria " << results[i] << std::endl;
    ...
    for (auto rms_factor: rms_factors){
      ...
      if (flag_trim_in_range){ // trimmed thresholds are equalizable!
        for (int channel_id = 0; channel_id < NCH_PER_VMM; channel_id++){ // channel loop
          ...
          // skip removed disconnected channels (ifany):                                                                                                                                
          if( channel_baseline_med.find(feb_ch) ==  channel_baseline_med.end() )
            continue;
        ...
        mean_best_trim = sum_best_trim/number_of_con_channels;
        thdac = calculate_thdac_value(cs,feb,vmm_id,n_samples,thdac_central_guess,thdac_guess_variations,mean_best_trim);
        std::cout << "Sum of best trimmers for " << feb.getAddress() <<" is:" << sum_best_trim << " with mean "<< mean_best_trim << " connected channels "<< number_of_con_channels << std::endl;
