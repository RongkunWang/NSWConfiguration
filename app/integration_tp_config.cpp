...
int main(int argc, const char *argv[]) 
{
    ...
    // //0: try to set them to 1 before everything
    // std::cout << "step 0" << std::endl;
    // cs.sendGPIO(opc_ip, sca_address + ".gpio.art0SRstn", 1);
    // cs.sendGPIO(opc_ip, sca_address + ".gpio.art1SRstn", 1);
    // cs.sendGPIO(opc_ip, sca_address + ".gpio.art0CRstn", 1);
    // cs.sendGPIO(opc_ip, sca_address + ".gpio.art1CRstn", 1);
    // cs.sendGPIO(opc_ip, sca_address + ".gpio.art0Rstn", 1);
    // cs.sendGPIO(opc_ip, sca_address + ".gpio.art1Rstn", 1);
    ...
    // sleep(1);
    // cs.sendGPIO(opc_ip, sca_address + ".gpio.gbtx0Rstn", 1);
    // cs.sendGPIO(opc_ip, sca_address + ".gpio.gbtx1Rstn", 1);
    ...
    cs.sendI2cRaw(opc_ip, sca_address + ".i2cMaster0", data.data(), data.size() );

    // implement some reading of a register via I2C just to test
    std::vector<uint8_t> outdata = cs.readI2cAtAddress(opc_ip, sca_address + ".i2cMaster0", data.data(), data.size());
