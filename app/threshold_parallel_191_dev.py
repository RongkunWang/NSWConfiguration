# 191 version of threshold_parallel.py: add option to specify the output directory (otherwise multiple jobs overwrite eachother's outputs...)
...
def options():
    ...
    parser.add_argument("--remove_outliers",	   help="Pass --remove_outliers to the script", action="store_true")
    parser.add_argument("--remove_disconnected",	   help="Pass --remove_disconnected to the script", action="store_true")
    parser.add_argument("-o",	                   help="Output files directory")
...
def main():
    ...
    if ops.remove_disconnected:
        print("Disconnected strips will be ignored in the estimation of the VMM thresholds.")
    else:
        print("Disconnected strips will be taken into account in the estimation of the VMM thresholds.")
    ...
    cfg["config"] = ops.c
    cfg["script"] = ops.s
    cfg["remove_outliers"] = "--remove_outliers" if ops.remove_outliers else ""
    cfg["remove_disconnected"] = "--remove_disconnected" if ops.remove_disconnected else ""
    cfg["out_dir"] = ops.o+"/" if ops.o else ""
...
def process_vmms(cfg, feb):
    ...
    for i in range(VMM_NUM):
       ...
       this_cfg["vmm"] = i
       outdir = cfg["out_dir"]
       this_cfg["out"] = "%sthresholds_%s_vmm_%i.txt" % (outdir,feb, i)
...
def process_baselines(cfg):
    ...
    cmd = "%(script)s -c %(config)s -n        %(feb)s -V %(vmm)s -o %(out_dir)s --rms_factor %(rms)s %(remove_outliers)s %(remove_disconnected)s " % (cfg) +\
