...
int main(int argc, char **argv) {
    ...
    try {
        ...
        const std::string segmentName = cmdParser.segmentName();
        const std::string partitionName = cmdParser.partitionName();

        // Instantiate ItemCtrl with proper Controllable
        daq::rc::ItemCtrl itemCtrl(cmdParser, std::shared_ptr<daq::rc::Controllable>(new nsw::NSWRecoveryControllerRc(partitionName, segmentName)));
        itemCtrl.init();
        itemCtrl.run();
    }
