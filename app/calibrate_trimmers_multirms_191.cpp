...
// 191 version. Updates:
// 2020/12/11: Henri added an option to specify the directory to which the baselines*txt files are written.
// 2021/01/28: Henri added an option to remove disconnected strips.
...
bool check_disconnected_channel(const char mmfe8[15], int vmm_id, int channel_id){
    
    int I,J,K; // Name format: MMFE8_L1P6_IPL
    ...
    if(((layer==1 || layer==3) && quad=='H')  || ((layer==2 || layer==4) && quad=='I')){
        if (side=='R') {I = 2*(pcb-1); J=7-vmm_id; K=63-channel_id;}
        else           {I = 2*(pcb-1)+1; J=vmm_id; K=channel_id;}
    }
    
    else {
        if (side=='L') {I = 2*(pcb-1); J=7-vmm_id; K=63-channel_id;}
        else           {I = 2*(pcb-1)+1; J=vmm_id; K=channel_id;}}
    
    int i(I*512+J*64+K);

    if(i%1024==1023 || i%1024==0) return true;

    int L(layer-1); // IP1 = layer0
    if(quad=='H')L=7-L; // HO1 = layer7

    if((L == 0 || L == 1 || L == 6 || L == 7) && (i<42 || (i>5078 && i<5149) || i>8160)) return true;
    if((L == 2 || L == 3 || L == 4 || L == 5) && (i<35 || (i>5108 && i<5121))) return true;
    //           if((L == 2 || L == 3 || L == 4 || L == 5) && (i>5080 && i<5155)) return true; // not sure what this line is/used to be
...
int main(int ac, const char *av[]) {
  ...
  bool remove_disconnected_channels = 0;
  ...
  desc.add_options()
      ...
      ("remove_disconnected", po::bool_switch()->default_value(false), "Remove disconnected channels from the baseline measurement")
      ("outdir,o", po::value<std::string>(&output_dir)->default_value("."),
  ...
  remove_disconnected_channels = vm["remove_disconnected"].as<bool>();
  ...
  for (auto & feb : frontend_configs) { // big feb loop
    ...
    if (remove_disconnected_channels) {
      std::cout << "INFO - Will remove disconnected channels from threshold estimation" << std::endl;
    }
    ...
    // write baseline
    std::ofstream myfile;
    myfile.open(output_dir+"/baselines_" + feb.getAddress() + "_VMM" + std::to_string(vmm_id) + ".txt");
    ...
    for (int channel_id = 0; channel_id < NCH_PER_VMM; channel_id++){// channel loop
        ...
        if (remove_disconnected_channels && check_disconnected_channel(feb.getAddress().c_str(),vmm_id,channel_id) ) {
            if (debug){
                std::cout << "INFO removing disconnected channel "      << feb.getAddress()
                          << " vmm"       << vmm_id
                          << ", channel " << channel_id
                          << std::endl;
            }
    ...
    for (int channel_id = 0; channel_id < NCH_PER_VMM; channel_id++){
        // need to check if the channel is found in the map (when disconnected channels have been removed)
        std::pair<std::string,int> feb_ch(feb.getAddress(),channel_id);
        if( channel_baseline_med.find(feb_ch) !=  channel_baseline_med.end() ){
    ...
    for (auto rms_factor: rms_factors){
      ...
      if (first) {
        for (int channel_id = 0; channel_id < NCH_PER_VMM; channel_id++){
          ...
          std::pair<std::string,int> feb_ch(feb.getAddress(),channel_id);
          // skip removed disconnected channels (if any):
          if( channel_baseline_med.find(feb_ch) ==  channel_baseline_med.end() )continue; 
      ...
      if (flag_trim_in_range){ // trimmed thresholds are equalizable!
        for (int channel_id = 0; channel_id < NCH_PER_VMM; channel_id++){ // channel loop
          ...
          if (first) {
            // correct for channel-to-channel rms variations
            float ch_rms_corr = rms_factor * (channel_baseline_rms[feb_ch] - vmm_baseline_rms[feb.getAddress()]);
            // guess at a trim value
            float delta = channel_mid_eff_thresh[feb_ch] - vmm_mid_eff_thresh[feb.getAddress()] + ch_rms_corr;
