...
int main(int argc, const char *argv[]) 
{
    ...
    if (readPFEBBcidError) {
      std::ranges::for_each(hws, [](const auto& hw){
        const auto readout = hw.readPFEBBcidErrorReadout();
        const auto trigger = hw.readPFEBBcidErrorTrigger();
        std::cout << fmt::format("PFEB BCID error (readout): {:#010x}", readout) << std::endl;
        std::cout << fmt::format("PFEB BCID error (trigger): {:#010x}", trigger) << std::endl;
      });
    }
