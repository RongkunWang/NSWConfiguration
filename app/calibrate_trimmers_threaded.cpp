...
int main(int ac, const char *av[]) {
  ...
  for (auto & feb : frontend_configs) {
    ...
    // Copied from configure_frontend while rebasing to TDAQ9 JS 07/02/21
    // if (max_threads > 0) {
  ...
  for (auto& thread: *threads) {
    ...
    try {
      ...
      ERS_LOG("Trimmer calibration failed due to ers::Issue: " << ex.what());
    } catch (std::exception & ex) {
      ERS_LOG("Trimmer calibration failed due to std::exception: " << ex.what());
    }
  }
}
