...
int TRIM_LO = 0;
int TRIM_MID = 14;
float SLOPE_CHECK = 1/1000. * 4095.0;
...
int calculate_thdac_value(nsw::ConfigSender & cs,
  ...
  for (unsigned int i = 0; i < thdac_guess_variations.size(); i++){
    ...
    feb.getVmm(vmm_id-vmm_start).setGlobalThreshold((size_t)(thdac_guess_variations[i]));
...
int main(int ac, const char *av[]) {
  ...
  if (fe_name != "") {
    ...
    if(fe_name.find("SFEB6") != std::string::npos) vmm_start = 2;
  } else {  // If no name is given, find all elements
    frontend_names = reader1.getAllElementNames();
  }

  std::cout << "Printing vmm_start value : " << vmm_start << std::endl;
  ...
  for (auto & name : frontend_names) {
    //std::cout << "The input FEB name is " << name << std::endl;
    try {
