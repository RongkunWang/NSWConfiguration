...
void nsw::ConfigSender::sendAddcConfig(const nsw::ADDCConfig& addc) {
    ...
    // art phase shifter config
    ERS_DEBUG(1, "ART phase shifter");
    for (auto art: addc.getARTs()) {
        // auto tup = std::make_pair("Ps",   art.ps);
        // auto name = sca_addr + "." + art.getName() + tup.first + "." + art.getName() + tup.first;
        // auto addr_bitstr = tup.second.getBitstreamMap();
        // ERS_DEBUG(1, "ART phase shifter " << name);

        // std::cout << "Starting with channel 0" << std::endl;
        ...
        // art_data[0] = 0x0a;
        // art_data[1] = 0x00;
        // auto outputPhase = readI2cAtAddress(opc_ip, name, art_data, 1, 1)[0];
        // std::cout << "ART output phase: " << (uint) outputPhase << std::endl;
        ...
        // ERS_DEBUG(2, "Waiting for ART PhaseAligner Pll locks...");
        // usleep(art.TP_GBTxAlignmentSleepTime());
        // bool locked = 0;
        // while (!locked) {
        //     art_data[0] = 0x0e;
        //     art_data[1] = 0x00;
        //     auto lockedBits = readI2cAtAddress(opc_ip, name, art_data, 1, 1)[0];
        //     locked = lockedBits==0xff;
        //     std::cout << "ART Lock: " << (uint) lockedBits << std::endl;


        //     art_data[0] = 0x0a;
        //     art_data[1] = 0x00;
        //     outputPhase = readI2cAtAddress(opc_ip, name, art_data, 1, 1)[0];
        //     std::cout << "ART output phase: " << (uint) outputPhase << std::endl;

        //     // readI2cAtAddress(opc_ip, name, uint8_t* address, size_t address_size, size_t number_of_bytes)
        //     // bool rPll1 = readGPIO(opc_ip, address + ".gpio.rocPllLocked");
        // }
        // // ERS_DEBUG(2, "rocPllLocked: " << rPll1 << ", rocPllRocLocked: " << rPll2);
...
void nsw::ConfigSender::alignAddcGbtxTp(const nsw::ADDCConfig& addc) {
    ...
    // the TP register for reseting
    std::string regAddrRst = "0x03";
    auto regAddrVecRst = nsw::hexStringToByteVector(regAddrRst, 4, true);
    auto data_enable   = nsw::hexStringToByteVector("0001",     4, true);
    auto data_disable  = nsw::hexStringToByteVector("0000",     4, true);
    std::vector<uint8_t> entirePayloadEnable (regAddrVecRst);
    std::vector<uint8_t> entirePayloadDisable(regAddrVecRst);
    entirePayloadEnable .insert(entirePayloadEnable .end(), data_enable .begin(), data_enable .end() );
    entirePayloadDisable.insert(entirePayloadDisable.end(), data_disable.begin(), data_disable.end() );
    for (auto art: addc.getARTs()){
        if (art.TP_GBTxAlignmentSkip())
            break;
        // ERS_DEBUG(1, addc.getAddress() << "/" << art.getName() << " Resetting TP reg 0x03...");
        // sendI2cRaw(art.getOpcServerIp_TP(), art.getOpcNodeId_TP(), entirePayloadEnable .data(), entirePayloadEnable .size()); usleep(art.TP_GBTxAlignmentSleepTime());
        // sendI2cRaw(art.getOpcServerIp_TP(), art.getOpcNodeId_TP(), entirePayloadDisable.data(), entirePayloadDisable.size()); usleep(art.TP_GBTxAlignmentSleepTime());
        // ERS_DEBUG(1, addc.getAddress() << "/" << art.getName() << " Finished resetting TP reg 0x03");
        break;
    }
    ...
    for (auto art: addc.getARTs()){
        ...
        while (n_attempts < max_attempts) {
            ...
            for (auto phase: art.TP_GBTxAlignmentPhasesToTest()) {
                ...
                // // addc readback
                // size_t addr_size_readback = 1;
                // uint8_t addr_readback[]   = {0x0};
                // auto val = readI2cAtAddress(addc.getOpcServerIp(),
                //                             addc.getAddress() + "." + art.getNameGbtx(),
                //                             addr_readback,
                //                             addr_size_readback);
                // std::cout << "Readback: " << unsigned(val[0]) << std::endl;
