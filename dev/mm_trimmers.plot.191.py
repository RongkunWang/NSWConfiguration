...
def plot(feb, outfile, tr_bl, tr_th_pre, tr_th_post):
    ...
    for bin in range(1,h1_df_pre.GetNbinsX()+1):
        if( h1_bl.GetBinContent(bin) > 1e-9 ):
            ...
            h1_df_pre .SetBinContent(bin, 0)
            h1_df_post.SetBinContent(bin, 0)
    ...
    # legend
    legend = ROOT.TLegend(0.2, 0.33, 0.79, 0.51)
    style(legend)
    ...
    tboxes = []
    for bin in range(1,h1_bl.GetNbinsX()+1):
        if( h1_bl.GetBinContent(bin) < 1e-9 ):
            b = ROOT.TBox(h1_bl.GetBinLowEdge(bin), 0, h1_bl.GetBinWidth(bin)+h1_bl.GetBinLowEdge(bin), 5)
            b.SetFillColor(2)
            b.Draw()
            tboxes.append(b)
    legend.Draw()
    ...
    canv = ROOT.TCanvas("%s_unzoom" % (feb), 
                        "%s_unzoom" % (feb), 800, 800)
    canv.Draw()
    h1_bl     .Draw("pesame")
    h1_th_pre .Draw("pesame")
    h1_th_post.Draw("pesame")
