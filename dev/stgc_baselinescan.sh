...
CONFIG="${FILES}/flx2layer1pFEB.json"
NSAMPS="100"
...
#time python NSWConfiguration/app/threshold_parallel.py -rms $RMS -c $CONFIG -s $CALIBR -f $FEBs
...
for R in $(echo ${RMS} | tr "," "\n"); do
   ...
   # $BIN/read_channel_monitor -c $CONFIG -s  10 -o $OUT/$R/thresholds_trimmed   --threshold --dump -n $FEBs
   # $BIN/read_channel_monitor -c $CONFIG -s  10 -o $OUT/$R/thresholds_untrimmed --threshold --dump -n $FEBs -T 0
