...
#MMFE8S="MMFE8_L2P8_HOL"
#,MMFE8_L1P7_HOL,MMFE8_L3P7_HOL,MMFE8_L4P7_HOL"
...
############# CONFIG="/afs/cern.ch/user/n/nswdaq/public/sw/config-ttc/config-files/config_json/BB5/A01/full_large_sector_a01_bb5_cosmics.json"
CONFIG="/afs/cern.ch/user/n/nswdaq/public/sw/config-ttc/config-files/config_json/BB5/A07/full_large_sector_a07_bb5_baselines.json"
...
CALIBR="${BIN}/calibrate_trimmers_multirms_old"
...
time python NSWConfiguration/app/threshold_parallel.py -rms $RMS -c $CONFIG -s $CALIBR -f $MMFE8S -m $THREADS
...
for R in $(echo ${RMS} | tr "," "\n"); do
    ...
    cp $OUT/trimmers_${NOW}_x${R}.root          ${WEB}/
    cp $OUT/trimmers_x${R}.json                 ${WEB}/
    cp $OUT/baselines.txt                       ${WEB}/
    cp $OUT/trimmers_x${R}.txt                  ${WEB}/
