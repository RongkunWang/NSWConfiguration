...
def options():
    ...
    parser.add_argument("-t", help="Type of chamber", default="LM")                                                                                                                                          
...
def main():
    ...
    if ops.t == "LM":
        type_chamber = "LM"
    if not ops.t in ["SM","LM"]:
        fatal("Please give right type of chamber (capital letters only)")
    ...
    for feb in febs:
        ...
        number_miss_channels = missing_channels(name,"LM")
        for bin in range(1, p_sigma.GetNbinsX()+1):
                ...
                if 513 - bin > number_miss_channels:
                    ...
                    if p_sigma.GetBinContent(bin) == missing_channels_distr.GetBinContent(bin):#down_limit_sigma:
                        count_down_sigma = count_down_sigma - 1
    ...
    print(out)
    print("The root file is CLOSED")

    out = ROOT.TFile.Open(ops.o, "UPDATE")

    out.mkdir("elx_report")
    outdir3 = out.GetDirectory("elx_report")

    os.system("root -l -b nn_l/run.cpp  -q")    
#    print(os.getenv["counter"])
#    ops = options()
#    os.environ["run"] = "%s" % run()
#    print("++++++++++++ %s" % (counter))

#    if ops.t == "SM":
#        os.system("root -l -b nn/run.cpp  -q")
#    if ops.t == "LM":
#        os.system("root -l -b nn_l/run.cpp  -q")
    ...
    fname = "Baselines.root"
    rfile = ROOT.TFile.Open(fname, "read")

    print("fname is %s" % (fname))
    if not rfile:
        fatal("Couldnt open %s" % (fname))
    for i in range(0, 8):
        el[i] = rfile.Get("Layer_%i" % i)
        ll[i] = rfile.Get("Layer_lin_%i" % i)
    dc = rfile.Get("dc+nc")
    rfile.Close()
...
def missing_channels(name,type_chamber):
